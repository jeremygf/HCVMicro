...
// ADDED BY JGF: Keep track of the current key we're in to avoid the performance hit 
// associated with looking up the key for every value we add (VERY expensive)
  unsigned curKeyID = noID;
...
// ADDED BY JGF: Keep track of the current key we're in to avoid the performance hit 
// associated with looking up the key for every value we add (VERY expensive) 
	// Used to use the following line but it has a VERY high performance hit for large files
	//	  SetValue( keyname, valuename, value);
	// Use this instead
	  if (haveKeyID == true) {
		  SetValue( curKeyID, valuename, value);
	  } else {
	    // We have an error! So we return out of it
		return false;
	  }
// END ADD
	  break;
...
{
// ADDED BY JGF: Performance for lookups
  map< string, unsigned >::const_iterator find_val = sorted_keys.find(CheckCase(keyname));
  if (find_val == sorted_keys.end()) {
	return noID;
  } else {
	return long(find_val->second);
  }
// END ADD
...
// ADDED BY JGF: Performance for lookups
  map< string, unsigned >::const_iterator find_val = keys[keyID].sorted_names.find(CheckCase(valuename));
  if (find_val == keys[keyID].sorted_names.end()) {
	return noID;
  } else {
	return long(find_val->second);
  }
...
{
  // ADDED BY JGF: Performance for lookups
  sorted_keys[CheckCase(keyname)]=keys.size();
  // END ADD
...
// ADDED BY JGF: Keep track of the current key we're in to avoid the performance hit 
// associated with looking up the key for every value we add (VERY expensive) 
bool CIniFile::SetValue( unsigned const keyID, string const valuename, string const value, bool const create)
{
  long valueID = FindValue( unsigned(keyID), valuename);
  if ( valueID == noID) {
    if ( !create)
      return false;
	keys[keyID].sorted_names[CheckCase(valuename)]=keys[keyID].names.size();
...
// ADDED BY JGF: Performance for lookups 
  keys[keyID].sorted_names.erase(CheckCase(valuename));
  for(map<string, unsigned>::iterator i = keys[keyID].sorted_names.begin(); i != keys[keyID].sorted_names.end(); i++) {
	  if (i->second > valueID) {
		  i->second--;
	  }
  }
...
// ADDED BY JGF: Performance for lookups
  sorted_keys.erase(CheckCase(keyname));
  for(map<string, unsigned>::iterator i = sorted_keys.begin(); i != sorted_keys.end(); i++) {
	  if (i->second > keyID) {
		  i->second--;
	  }
  }
